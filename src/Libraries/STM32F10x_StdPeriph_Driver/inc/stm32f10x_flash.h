...
{ 
  FLASH_BUSY = 1,		//Ã¦
  FLASH_ERROR_PG,		//´íÎó±à³Ì
  FLASH_ERROR_WRP,		//´íÎóÐ´±£»¤
  FLASH_COMPLETE,		//ÏÐ
  FLASH_TIMEOUT			//³¬Ê±
}FLASH_Status;
...
FLASH_Status FLASH_ErasePage(uint32_t Page_Address);//²Á³ýÒ³
FLASH_Status FLASH_EraseAllPages(void);//²Á³ýËùÓÐÒ³
FLASH_Status FLASH_EraseOptionBytes(void);
FLASH_Status FLASH_ProgramWord(uint32_t Address, uint32_t Data);
FLASH_Status FLASH_ProgramHalfWord(uint32_t Address, uint16_t Data);
FLASH_Status FLASH_ProgramOptionByteData(uint32_t Address, uint8_t Data);
FLASH_Status FLASH_EnableWriteProtection(uint32_t FLASH_Pages);//Ê¹ÄÜÐ´±£»¤
